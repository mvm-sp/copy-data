...
let rawdata = fs.readFileSync('source-query.json');  
...
console.log = function(...args) {
    ...
    var op = args.join(' ');
    output.write(new Date().toLocaleString() + ' -[GETFROMCLINUX]- ' + util.format(op) + '\r\n');
};
...
console.error = function(...args) {
    ...
    var op = args.join(' ');
    errorOutput.write(new Date().toLocaleString() + ' -[GETFROMCLINUX]- ' + util.format(op) + '\r\n');
};
...
function writeFileDBDirect(){
    ...
    console.log('{' + clientData.name  + '}' +'Begin writeFileDBDirect...');
    var _sqlParam = '';
    var localQuery = fs.readFileSync('./sql/' + runParam.query, 'utf8');
