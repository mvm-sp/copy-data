...
/*
Endereço: fioravante.ecoclinica.com.br
Porta: 5555
Usuário:mouraconsultoria
Senha:moura#32$eco
Liberado para: 191.8.187.124, 187.34.156.100

glbConnString = "DRIVER={PostgreSQL Unicode(x64)};Server=192.168.0.60;Port=5432;Database=clinux_cdi;Uid=dicomvix;Pwd=system98"


*/
var connTest ={ 
    host: 'fioravante.ecoclinica.com.br',
    database: 'clinux_ecoclinica', 
    port: 5555, 
    user: 'mouraconsultoria',
    password: 'moura#32$eco'
} ;
...
const clientPG = new Client(connTest);
...
function writeFileDBDirect(){
    ...
    //localQuery = 'SELECT * FROM examprocedureext WHERE idclient = 19'
    ...
    var commandPsql = clientData.psql + 'COPY (' + localQuery + ') TO  STDOUT  with CSV DELIMITER \';\' HEADER';
    console.log('{' + clientData.name  + '}' +' Connect using : ', crypt(connSettings));
    
    const pool = new Pool(connTest);
    ...
    pool.connect(function(pgErr, client, done) {
        ...
        console.log('{' + clientData.name  + '}' +' Trying to execute command: ', commandPsql);
        var stream = client.query(copyTo(commandPsql));
        var pipe = stream.pipe(writer);
        pipe.on('finish', function () {
            console.log('{' + clientData.name  + '}' +'Finish read data ');
            done();
        });
...
const ExecuteTaskInHost = ()=>{
    ...
    var sQuery = 'SELECT * FROM prgetjsonclientdatabase(' + args + ')';
    var hostDatta = {"idClient" : 19, "name" : "Cliente_Teste", "connSettings" : {"host":"clinicadelfos.zapto.org","port":"1157","username":"dicomvix","password":"SysteM98"}, "remoteDir" : "data/teste.csv", "psql" : ""};
    clientData = hostDatta;
